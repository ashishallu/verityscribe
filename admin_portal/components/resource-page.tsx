    ...
    <div className="min-h-screen bg-gradient-to-br from-black to-slate-900 text-slate-100">
      <div className="container max-w-7xl mx-auto px-4 py-8">
        ...
        <Card className="bg-slate-900/50 border-slate-700/50 backdrop-blur-sm"><CardHeader><CardTitle>{title}</CardTitle><CardDescription>{total} total records</CardDescription></CardHeader><CardContent>
          {loading ? <div className="text-center py-8">Loading...</div> : error ? <div className="text-center py-8 space-y-3"><p className="text-red-300">{error}</p><Button onClick={() => void reload()}>Retry</Button></div> : data.length === 0 ? <div className="text-center py-8 text-slate-400">No {resource} records are available for this account.</div> : <AdvancedTable columns={columns} data={data} title={title} onDelete={allowDelete ? (row) => void remove(row.id) : undefined} />}
        </CardContent></Card>
      </div>
    </div>
  </ProtectedRoute>
